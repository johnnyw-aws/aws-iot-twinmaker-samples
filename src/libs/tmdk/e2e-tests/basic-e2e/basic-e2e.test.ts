...
test("basic e2e test", async () => {
  ...
  const tmdkDefinition1 = JSON.parse(
    fs.readFileSync(
      path.join(constants.tmdkDirectory, "tmdk.json"),
      constants.jsonEncoding
    )
  );
  ...
  const entities1Definition = JSON.parse(
    fs.readFileSync(
      path.join(constants.tmdkDirectory, "entities.json"),
      constants.jsonEncoding
    )
  );
  ...
  fs.writeFileSync(
    path.join(constants.tmdkDirectory, constants.scene2FileName),
  ...
  const entities = JSON.parse(
    fs.readFileSync(
      path.join(constants.tmdkDirectory, constants.entitiesFile),
      constants.jsonEncoding
    )
  );
  ...
  const tmdkDefinition = JSON.parse(
    fs.readFileSync(
      path.join(constants.tmdkDirectory, constants.tmdkFile),
      constants.jsonEncoding
    )
  );
