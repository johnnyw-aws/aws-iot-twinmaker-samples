...
test("deploys nothing when given an empty tmdk project", async () => {
  fs.existsSync = jest.fn(() => true );
...
test("deploys successfully when given a tmdk project with one component type", async () => {
  fs.existsSync = jest.fn(() => true );
  fs.readFileSync = jest.fn((path: [string]) => {
    if (path.includes("tmdk")) {
...
test("deploys successfully when given a tmdk project with one scene with one model", async () => {
  fs.existsSync = jest.fn(() => true );
  fs.readFileSync = jest.fn((path: [string]) => {
    if (path.includes("tmdk")) {
...
test("deploys successfully when given a tmdk project with one entity", async () => {
  fs.existsSync = jest.fn(() => true );
  fs.readFileSync = jest.fn((path: [string]) => {
    if (path.includes("tmdk")) {
      const tmdk1Ct = JSON.parse(JSON.stringify(emptyTmdk));
