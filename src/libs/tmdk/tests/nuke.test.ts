...
test("exits without deleting anything when not given 'Y' input", async () => {
  prompts.inject(["n"]);
...
test("deletes nothing when given an empty workspace", async () => {
  prompts.inject(["Y"]);
  twinmakerMock.on(GetWorkspaceCommand).resolves({});
  twinmakerMock
...
test("deletes 1 entity when given a workspace with 1 entity", async () => {
  prompts.inject(["Y"]);
  twinmakerMock.on(GetWorkspaceCommand).resolves({});
  twinmakerMock
    .on(ListComponentTypesCommand)
...
test("deletes 1 component type when given a workspace with 1 component type", async () => {
  prompts.inject(["Y"]);
  twinmakerMock.on(GetWorkspaceCommand).resolves({});
  twinmakerMock
    .on(ListComponentTypesCommand)
...
test("deletes 1 scene when given a workspace with 1 scene", async () => {
  prompts.inject(["Y"]);
  twinmakerMock.on(GetWorkspaceCommand).resolves({});
  twinmakerMock
    .on(ListComponentTypesCommand)
...
test("deletes all twinmaker resources when given a populated workspace", async () => {
  prompts.inject(["Y"]);
  twinmakerMock.on(GetWorkspaceCommand).resolves({});
  twinmakerMock
    .on(ListComponentTypesCommand)
