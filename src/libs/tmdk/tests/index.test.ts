...
describe('testing audit file', () => {
  ...
  test("test02", () => {
    var argv2 = {
      ...
      "grafana-workspace-id": "grafana-url"
    } as Arguments<Options>;
    handler(argv2)
    return expect(mockExit).toHaveBeenCalledWith(1);
  })
  ...
  test("test03", () => {
    ...
    return expect(handler(argv2)).rejects.toEqual(new Error('Failed to get workspace. Error: mock error'));
  })
});