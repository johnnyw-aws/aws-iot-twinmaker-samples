...
async function import_component_types(
  ...
  const resp = await aws().tm.listComponentTypes({
  ...
  if (componentTypeSummaries != undefined) {
    for (const componentTypeSummary of componentTypeSummaries) {
      if (!componentTypeSummary.arn?.includes("AmazonOwnedTypesWorkspace")) {
        ...
        const compRespObj = compResp["propertyDefinitions"] as object;
        let filtered_property_definitions;
        if (compRespObj != undefined) {
          filtered_property_definitions = Object.entries(compRespObj).reduce(
            (acc, [key, value]) => {
        ...
        (tmdk_config["component-types"] as string[]).push(
...
async function import_scenes_and_models(
  ...
  const res2 = await aws().tm.listScenes({ workspaceId: workspaceIdStr }); // FIXME pagination
  // console.log(res2);
...
export const handler = async (argv: Arguments<Options>) => {
  ...
  let entityCount = 0;
  while (true) {
    res2 = await aws().tm.listEntities({
      workspaceId: workspaceIdStr,
      nextToken: nextToken,
    }); // FIXME pagination
