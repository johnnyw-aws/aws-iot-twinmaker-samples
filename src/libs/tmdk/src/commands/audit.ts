...
export const command: string = 'audit';
export const desc: string = 'Scans IoT TwinMaker resources for issues (missing permissions, outdated plugin version, unused resources, etc). Explanation analysis for entity/component properties (e.g. inheritance path, isAbstract causes).';
    ...
    if (workspaceId) {
      try {
        ...
        if (e instanceof ResourceNotFoundException) {
          console.log(`Error: workspace '${workspaceId}' not found in region '${region}'. Please create it first.`)
          process.exit(1);
        } else {
