...
async function syncEntities(
  ...
  for (const [i, entityDefinition] of entityDefinitions.entries()) {
    if (!entityDefinition.isProcessed) {
      if (
        await recursiveSyncEntity(
          workspaceId,
          entityDefinition,
          entityDefinitions.slice(i)
        )
      ) {
        numEntitiesSynced++;
      } else {
  ...
  if (entityId == undefined) {
    ...
    try {
      ...
      if (
        ...
        e.message.includes("Unable to find parent entity id")
      ) {
        // recursive case:
        if (entityDefinition.parentEntityId != undefined) {
          const parent = findParentEntityDefinition(
            entityDefinitions,
            entityDefinition.parentEntityId,
            undefined
          );
          if (parent != undefined) {
            // sync parent first. If sucessful, we can retry child
            if (
              await recursiveSyncEntity(workspaceId, parent, entityDefinitions)
            ) {
              numEntitiesSynced++;
              await recursiveSyncEntity(
                workspaceId,
  ...
  if (
    ...
    try {
      ...
      if (
        ...
        existingBody["parentEntityId"] !== parentEntityId
      ) {
        if (parentEntityId == "") {
...
function findParentEntityDefinition(
  ...
  for (const entityDefinition of entityDefinitions) {
    if (
      (typeof entityDefinition.entityId == "string" &&
        entityDefinition.entityId == parentEntityId) ||
