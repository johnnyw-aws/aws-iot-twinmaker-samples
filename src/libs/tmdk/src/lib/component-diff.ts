...
function componentsNeedUpdates(
    ...
    let oldComponents = existingBody["components"];
    if (oldComponents != undefined) {
        for (let key in componentUpdates) {
            // check that every ComponentUpdateRequest key exists
            if (oldComponents[key] != undefined) {
                let values = componentUpdates[key];
                let oldValues = oldComponents[key];
                ...
                if (
                    ...
                    let oldProperties = oldValues.properties;
                    for (let propertyKey in newProperties) {
                        // compare every new PropertyRequest
                        let newProperty = newProperties[propertyKey];
                        if (oldProperties != undefined) {
                            let oldProperty = oldProperties[propertyKey];
                            ...
                            if (JSON.stringify(newProperty.value) != JSON.stringify(oldProperty.value)
...
function propertyDefinitionIsEqual(
    ...
    if (newDefinition != undefined && oldDefinition != undefined) {
        ...
        const keys: (keyof PropertyDefinitionRequest)[] =
            ["isRequiredInEntity", "isExternalId", "isStoredExternally", "isTimeSeries"];
        ...
        if (newDefinition.configuration != undefined) {
            ...
            if (oldDefinition.configuration == undefined) {
                ...
                for (let key in newDefinition.configuration) {
...
function dataTypeIsEqual(
    ...
    if (newDataType != undefined && oldDataType != undefined) {
        ...
        } if (newDataType.unitOfMeasure != undefined
